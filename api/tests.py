...
class BookReviewAPITestCase(APITestCase):
    def setUp(self):
        self.user = CustomUser.objects.create(username="zarif", first_name="zarifxon")
        self.user.set_password("somepass")
        self.user.save()
        self.client.login(username="zarif", password="somepass")

    def test_book_review_detail(self):
        book = Book.objects.create(title='Book1', description='Description1', isbn="122112213")
        ...
        self.assertEqual(response.data['book']['isbn'], '122112213')
        self.assertEqual(response.data['user']['id'], self.user.id)
        self.assertEqual(response.data['user']['first_name'], 'zarifxon')
        self.assertEqual(response.data['user']['username'], 'zarif')
